...
function indent(depth, baseDepth, multip, ch, empty) {
    baseDepth = !isNaN(parseInt(baseDepth)) ? parseInt(baseDepth) : 0;
    depth = (!isNaN(parseInt(depth)) ? parseInt(depth) : 0) + baseDepth
    multip = !isNaN(parseInt(multip)) ? parseInt(multip) : 2;
    ch = ch || ' ';
    empty = empty !== undefined ? empty : '';
    return depth ? new Array(depth * multip + 1).join(ch) : empty;
}

function stringContains(needle, haystack, strict) {
    return strict ? diaStrip(haystack).indexOf(diaStrip(needle)) !== -1
            : diaStrip(haystack.toLowerCase()).indexOf(diaStrip(needle.toLowerCase())) !== -1;
}

function diaStrip(str) {

    var diacriticsCharMap = [
    ...
    diacriticsCharMap.forEach(function (char) {
        str = str.replace(char.letters, char.base);
    });
